...
camel {
    ...
    routeHandler {
		...
		LOG.info("Reading from an SFTP server :: '${dirName}/${fileName}' to '${pathStrInbound}/{{appName}}'")
        from("sftp:${endPointPath}/${dirName}?username=${userName}&password=${password}&include=${fileName}" +
            ...
            .log("{{upperCase appName}} file transfer: \${headers.CamelFileName}")
            .to("file:${pathStrInbound}")
        LOG.info("Done - Reading from an SFTP server :: '${dirName}/${fileName}' to '${pathStrInbound}/{{appName}}'")
    }
}